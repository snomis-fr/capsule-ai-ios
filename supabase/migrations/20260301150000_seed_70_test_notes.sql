-- Jeu de 70 notes de test : titres percutants, 2-3 tags, ~20 lignes, types variés, répartition dans tous les espaces
-- Exécution : supabase db push ou SQL Editor
...
DECLARE
  ...
  plain TEXT;
BEGIN
  SELECT id, manager_id INTO ws_id, mgr_id FROM workspaces LIMIT 1;
  ...
  IF sub_ids IS NULL OR array_length(sub_ids, 1) < 1 THEN RETURN; END IF;

  SELECT ARRAY_AGG(tags.id) INTO tag_ids FROM tags WHERE tags.workspace_id = ws_id;
  ...
  FOR i IN 1..70 LOOP
    ...
    SELECT nid, tag_ids[1 + ((i + j) % array_length(tag_ids, 1))]
